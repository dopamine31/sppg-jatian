...
async function fetchJsonData(tableName) {
    try {
        ...
        return data || [];
    } catch (error) { console.error(`Error loading table ${tableName}:`, error); return []; }
}
...
function verifyPIN() {
    ...
    // Catatan: PIN hardcoded di sini. Untuk produksi, sebaiknya gunakan autentikasi Supabase.
    if (input === '2024') { 
...
async function loadSekolah() {
    ...
    tbody.innerHTML = data.map((row, i) => {
        ...
        return `<tr>
            <td>${i + 1}</td><td><span class="npsn-badge">${npsn}</span></td><td><strong>${jenjang}</strong></td>
            <td><strong>${namaSekolah}</strong></td><td>${statusBadge}</td><td>${kecamatan}</td>
            <td>${kelDesa}</td><td style="max-width:200px;">${alamat}</td><td style="text-align:center;"><strong>${totalPM}</strong></td>
            <td>${namaPIC}</td><td>${waCell}</td><td>${kepalaSekolah}</td><td>${noTelpCell}</td>
            <td>${emailCell}</td><td>${rekening}</td><td>${mapsCell}</td>
        </tr>`;
    }).join('');
...
async function loadRelawan() {
    ...
    tbody.innerHTML = data.map((row, i) => {
        ...
        return `<tr>
            <td>${i + 1}</td><td><strong>${escapeHtml(row['Nama'])}</strong></td>
            <td class="${nikClass}">${nikDisplay}</td><td>${waCell}</td>
            <td>${escapeHtml(row['Email'])}</td><td>${escapeHtml(row['Posisi'])}</td><td>${ktpCell}</td>
        </tr>`;
    }).join('');
    
    // FIX: Tambahkan null check agar tidak crash jika elemen tidak ada
...
async function loadSurat() {
    ...
    if (searchSurat) searchSurat.addEventListener('input', (e) => { 
...
window.addEventListener('DOMContentLoaded', async () => {
    ...
    await loadAgenda();
    updateClock();
    document.addEventListener('keydown', (e) => { if (e.key === 'Escape') { if (sidebarOpen) toggleMenu(); if (document.getElementById('pinModal').classList.contains('active')) hidePINModal(); if (document.getElementById('ktpModal')?.classList.contains('active')) closeKTPModal(); } });
...
function jepretKamera() {
    ...
    // ==========================================
    // 2. HEADER ATAS KANAN (TANGGAL & HARI)
    // ==========================================
    const now = new Date();
    ...
    ctx.fillStyle = "#fbbf24"; // Hari Kuning Emas (lebih menonjol)
    ctx.font = `bold ${28 * scale}px Nunito, sans-serif`;
    ...
    // KOORDINAT - Kuning Emas (eye-catching)
...
// FIX: Menghapus stray closing brace '}' yang menyebabkan SyntaxError di baris paling bawah file asli.
