...
function maskNIK(nik) { 
    ...
    if (!checkAuth()) { 
        ...
        if (str.length > 4) return str.substring(0, 4) + '•'.repeat(str.length - 4); 
...
function showKTPModal(url, nama) { 
    ...
    if (!modal) { 
        ...
        modal.onclick = function(e) { if (e.target === modal) closeKTPModal(); }; 
        modal.innerHTML = `<div class="ktp-modal-content"><button class="ktp-modal-close" onclick="closeKTPModal()">✕</button><h3 class="ktp-modal-title">📷 KTP: <span id="ktpModalNama"></span></h3><img id="ktpModalImg" src="" alt="KTP"></div>`; 
...
function handleKTPClick(nama, ktpUrl) { 
    if (checkAuth()) { showKTPModal(ktpUrl, nama); } 
    else { 
        showPINModal(); 
        const checkInterval = setInterval(() => { if (checkAuth()) { clearInterval(checkInterval); showKTPModal(ktpUrl, nama); } }, 500); 
...
async function loadSekolah() { 
    ...
    if (data.length === 0) { tbody.innerHTML = '<tr><td colspan="16" style="text-align:center;padding:20px;">⚠️ Data tidak tersedia</td></tr>'; return; } 
    tbody.innerHTML = data.map((row, i) => { 
        ...
        const emailCell = isValidLink(row['Email']) ? `<a href="mailto:${row['Email'].trim()}" target="_blank" rel="noopener noreferrer">📧 ${email}</a>` : '<span style="color:#999;">-</span>'; 
        const noTelpClean = cleanWA(row['No Telp'] || row['No. Telp'] || row['No Telp Sekolah']); 
        const noTelpCell = noTelpClean ? `<a href="https://wa.me/${noTelpClean}" target="_blank" rel="noopener noreferrer">📞 ${noTelp}</a>` : '<span style="color:#999;">-</span>'; 
        const statusLower = (row['Status'] || row['Status Kepemilikan'] || '').toString().toLowerCase(); 
        let statusBadge = status; 
        if (statusLower.includes('negeri')) statusBadge = `<span style="background:linear-gradient(135deg,#22c55e,#15803d);color:#fff;padding:3px 10px;border-radius:10px;font-size:11px;font-weight:800;">NEGERI</span>`; 
        else if (statusLower.includes('swasta')) statusBadge = `<span style="background:linear-gradient(135deg,#f59e0b,#d97706);color:#fff;padding:3px 10px;border-radius:10px;font-size:11px;font-weight:800;">SWASTA</span>`; 
        return `<tr><td>${i + 1}</td><td><span class="npsn-badge">${npsn}</span></td><td><strong>${jenjang}</strong></td><td><strong>${namaSekolah}</strong></td><td>${statusBadge}</td><td>${kecamatan}</td><td>${kelDesa}</td><td style="max-width:200px;">${alamat}</td><td style="text-align:center;"><strong>${totalPM}</strong></td><td>${namaPIC}</td><td>${waCell}</td><td>${kepalaSekolah}</td><td>${noTelpCell}</td><td>${emailCell}</td><td>${rekening}</td><td>${mapsCell}</td></tr>`; 
    }).join(''); 
    const searchInput = document.getElementById('searchSekolah'); 
    if (searchInput) searchInput.addEventListener('input', (e) => filterTable(e, tbody)); 
}

async function loadRelawan() { 
    const tbody = document.querySelector('#tableRelawan tbody'); 
    if (!tbody) return; 
    ...
    tbody.innerHTML = data.map((row, i) => { 
        ...
        const ktpCell = row['Nama'] ? `<div class="ktp-container" onclick="handleKTPClick('${escapeHtml(row['Nama'])}', '${ktpUrl}')"><img src="${ktpUrl}" alt="KTP ${escapeHtml(row['Nama'])}" class="ktp-thumbnail" onerror="this.style.display='none'; this.nextElementSibling.style.display='block';"><span style="display:none;color:#666;font-size:10px;">📷 Belum ada</span></div>` : '<span style="color:#999;">-</span>'; 
    ...
    if (searchRelawan) searchRelawan.addEventListener('input', (e) => filterTable(e, tbody)); 
    loadBirthday(); 
}

async function loadKoordinator() { 
    const container = document.getElementById('cardKoordinator'); 
    if (!container) return; 
...
async function loadKontak() { 
    const container = document.getElementById('cardKontak'); 
    if (!container) return; 
    container.innerHTML = '<p style="text-align:center;padding:20px;">⏳ Memuat data...</p>'; 
...
async function loadSurat() { 
    const container = document.getElementById('cardSurat'); 
    if (!container) return; 
    container.innerHTML = '<p style="text-align:center;padding:20px;">⏳ Memuat data...</p>'; 
    ...
    if (searchSurat) searchSurat.addEventListener('input', (e) => { const q = e.target.value.toLowerCase(); container.querySelectorAll('.info-card').forEach(card => { card.style.display = card.textContent.toLowerCase().includes(q) ? '' : 'none'; }); }); 
}
...
async function loadDokumen() { 
    ...
    // ✅ Icon sudah ditambahkan dengan koma yang benar
    const icons = { sop: '📋', template: '📝', form: '📊', sk: '📜', panduan: '📖', mou: '🤝', internal: '' }; 
    ...
    grid.innerHTML = searchedData.map((doc) => { 
        ...
        if (doc['Tanggal Update']) { 
            ...
            tanggalUpdate = date.toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' }); 
        } 
        
        // ✅ Template literal sudah dibersihkan dari sisa kode error sebelumnya
        return `
        <div class="doc-item" data-category="${category}">
            ...
            <div class="doc-actions-grid">
                ${linkView ? `<button class="btn-doc-action btn-view" data-url="${linkView}" data-title="${escapeHtml(doc['Judul'])}" onclick="showPdfModal(this.dataset.url, this.dataset.title)"><i class="fas fa-eye"></i> Lihat</button>` : ''}
...
async function loadInfo() { 
    ...
    const data = await fetchJsonData('Papan Informasi'); 
...
async function loadMenuWeekly() { 
    const container = document.getElementById('menuWeeklyContainer'); 
    if (!container) return; 
...
async function loadQuote() { 
    const data = await fetchJsonData('quote'); 
    if (data.length > 0) { const random = data[Math.floor(Math.random() * data.length)]; document.getElementById('quoteText').textContent = `❝ ${random['Quote'] || random[Object.keys(random)[0]]} ❞`; } 
}

async function loadPengumuman() { 
    const data = await fetchJsonData('pengumuman'); 
    document.getElementById('runningText').textContent = data.length === 0 ? 'Tidak ada pengumuman.' : data.map(r => `${r['Judul'] || ''}: ${r['Isi'] || ''}`).join('   •   '); 
}

async function loadAgenda() { 
    const data = await fetchJsonData('agenda'); 
    ...
    agendaList.innerHTML = sorted.map(row => { 
        ...
        return `<div class="agenda-item"><div class="agenda-date"><div class="day">${day}</div><div class="month">${month}</div></div><div class="agenda-info"><h4>${escapeHtml(row['Kegiatan'])}</h4><p>${escapeHtml(row['Keterangan'])}</p></div></div>`; 
    }).join(''); 
}

async function loadBirthday() { 
    const data = globalData.relawan; 
    if (!data || data.length === 0) return; 
    const today = new Date(); 
    const todayMonth = today.getMonth() + 1; 
    const todayDay = today.getDate(); 
    const birthdayPersons = data.filter(row => { 
        const tglLahir = row['Tanggal Lahir']; 
        if (!tglLahir) return false; 
        const date = new Date(tglLahir); 
        return (date.getMonth() + 1) === todayMonth && date.getDate() === todayDay; 
    }); 
    if (birthdayPersons.length > 0) { 
        const names = birthdayPersons.map(p => p['Nama']).join(', '); 
        document.getElementById('birthdayText').textContent = `${names} 🎈`; 
        const firstPerson = birthdayPersons[0]; 
        const waNumber = cleanWA(firstPerson['Nomor WA']); 
        if (waNumber) { 
            const message = encodeURIComponent(`Halo ${firstPerson['Nama']}! 🎉 Selamat ulang tahun! Semoga sehat selalu. - Dari SPPG Jatian`); 
            document.getElementById('birthdayWaLink').href = `https://wa.me/${waNumber}?text=${message}`; 
...
window.addEventListener('DOMContentLoaded', async () => { 
    ...
    document.addEventListener('keydown', (e) => { 
        if (e.key === 'Escape') { 
            ...
            if (document.getElementById('ktpModal')?.classList.contains('active')) closeKTPModal(); 
            if (document.getElementById('pdfModalOverlay')?.classList.contains('active')) closePdfModal(); 
        } 
    }); 
...
async function startKameraGPS() { 
    ...
    if (navigator.geolocation) { 
        navigator.geolocation.watchPosition((pos) => { latKamera = pos.coords.latitude.toFixed(6); lngKamera = pos.coords.longitude.toFixed(6); }, (err) => { console.warn("GPS gagal:", err); }, { enableHighAccuracy: true }); 
    } 
    try { 
        const constraints = { video: { facingMode: { ideal: 'environment' }, width: { ideal: 2560, min: 1920 }, height: { ideal: 1440, min: 1080 } } }; 
        const stream = await navigator.mediaDevices.getUserMedia(constraints); 
        const videoEl = document.getElementById('kameraStream'); 
        videoEl.srcObject = stream; 
        streamKamera = stream; 
        cameraTrack = stream.getVideoTracks()[0]; 
        const capabilities = cameraTrack.getCapabilities ? cameraTrack.getCapabilities() : null; 
        if (capabilities && capabilities.zoom) { document.getElementById('zoomSlider').max = capabilities.zoom.max || 10; } 
        ...
        statusEl.innerText = "Kamera & GPS Siap"; 
    } catch (err) { statusEl.innerText = "❌ Izin kamera ditolak atau kamera tidak tersedia."; } 
}

async function detectCameraLenses() { 
    const devices = await navigator.mediaDevices.enumerateDevices(); 
    availableRearCameras = devices.filter(d => d.kind === 'videoinput' && (d.label.toLowerCase().includes('back') || d.label.toLowerCase().includes('rear') || d.label.toLowerCase().includes('environment'))); 
    ['0.5', '1', '2', '3'].forEach(zoom => { const btn = document.getElementById(`btnLens${zoom}x`); if (btn) { btn.disabled = true; btn.classList.remove('active'); } }); 
    const lensMap = { '0.5': /0\.5|ultra wide|wide angle/i, '1': /1x|wide|back camera(?!\d)/i, '2': /2x|telephoto|portrait/i, '3': /3x|telephoto/i }; 
    let foundLenses = []; 
    availableRearCameras.forEach(cam => { for (let [zoom, regex] of Object.entries(lensMap)) { if (regex.test(cam.label)) foundLenses.push({ zoom, deviceId: cam.deviceId }); } }); 
    ...
    foundLenses.forEach(lens => { const btn = document.getElementById(`btnLens${lens.zoom}x`); if (btn) { btn.disabled = false; if (lens.zoom === '1') btn.classList.add('active'); } }); 
}
...
async function switchLens(zoomLevel) { 
    ...
    for (let cam of availableRearCameras) { if (lensMap[zoomLevel] && lensMap[zoomLevel].test(cam.label)) { targetDevice = cam.deviceId; break; } } 
    if (!targetDevice && availableRearCameras.length > 1) { const index = ['0.5', '1', '2', '3'].indexOf(zoomLevel); if (index >= 0 && index < availableRearCameras.length) targetDevice = availableRearCameras[index].deviceId; } 
    if (targetDevice) { 
        try { await cameraTrack.applyConstraints({ deviceId: { exact: targetDevice } }); document.getElementById('zoomSlider').value = 1; document.getElementById('zoomValueDisplay').innerText = '1.0x'; } 
        catch (e) { console.warn("Gagal switch lensa fisik:", e); } 
    } else { applyDigitalZoom(parseFloat(zoomLevel)); } 
}
...
function applyDigitalZoom(zoomValue) { 
    ...
    cameraTrack.applyConstraints({ advanced: [{ zoom: zoomValue }] }).catch(e => console.warn("Zoom tidak didukung:", e)); 
}
...
function jepretKamera() { 
    ...
    ctx.shadowColor = "rgba(0, 0, 0, 0.95)"; ctx.shadowBlur = 12 * scale; ctx.shadowOffsetX = 3 * scale; ctx.shadowOffsetY = 3 * scale; 
    if (logoImg.complete && logoImg.naturalWidth !== 0) { ctx.shadowColor = "rgba(0, 0, 0, 0.8)"; ctx.shadowBlur = 10 * scale; ctx.drawImage(logoImg, padding, padding, 100 * scale, 100 * scale); } 
    ctx.fillStyle = "#ffffff"; ctx.font = `900 ${36 * scale}px Nunito, sans-serif`; ctx.textAlign = "left"; ctx.textBaseline = "middle"; 
    ...
    ctx.textAlign = "right"; ctx.fillStyle = "#ffffff"; ctx.font = `bold ${32 * scale}px Nunito, sans-serif`; 
    ctx.fillText(tanggalString, width - padding, padding + (40 * scale)); 
    ctx.fillStyle = "#fbbf24"; ctx.font = `bold ${28 * scale}px Nunito, sans-serif`; 
    ctx.fillText(hariString, width - padding, padding + (85 * scale)); 
    ctx.textAlign = "left"; ctx.textBaseline = "alphabetic"; ctx.fillStyle = "#ffffff"; ctx.font = `900 ${120 * scale}px Nunito, sans-serif`; 
    ctx.fillText(jamString, padding, height - (120 * scale)); 
    ctx.fillStyle = "#ffffff"; ctx.font = `bold ${34 * scale}px Nunito, sans-serif`; 
    ctx.fillText("Kabupaten Jember, Jawa Timur", padding, height - (65 * scale)); 
    ctx.fillStyle = "#fbbf24"; ctx.font = `bold ${30 * scale}px Nunito, sans-serif`; 
...
function kirimKeDrive() { 
    ...
    const statusEl = document.getElementById('kameraStatus'); 
    const canvasEl = document.getElementById('kameraCanvas'); 
    btnKirim.disabled = true; btnKirim.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Mengirim 2K...'; 
    ...
    fetch(GOOGLE_SCRIPT_URL, { method: 'POST', mode: 'no-cors', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }).then(() => { 
        statusEl.innerText = "✅ Berhasil! Foto 2K masuk ke Google Drive & Sheets."; 
        btnKirim.style.display = 'none'; btnKirim.disabled = false; btnKirim.innerHTML = '<i class="fas fa-cloud-upload-alt"></i> Kirim ke Drive'; 
    }).catch(err => { statusEl.innerText = "❌ Gagal mengirim. Pastikan internet stabil."; btnKirim.disabled = false; btnKirim.innerHTML = '<i class="fas fa-cloud-upload-alt"></i> Coba Kirim Lagi'; }); 
}
...
function toggleChat() { 
    ...
    if (!visible) { 
        chatOpened = true; unreadCount = 0; updateBadge(); 
        if (chatUser) { document.getElementById('chatNameForm').style.display = 'none'; document.getElementById('chatRoom').style.display = 'flex'; loadChat(); } 
    } 
}
...
function joinChat() { 
    ...
    if (name.toLowerCase().startsWith('admin')) chatIsAdmin = true; 
    chatUser = name; localStorage.setItem('redzoneUser', name); 
    document.getElementById('chatUserLabel').textContent = name; 
...
async function loadChat() { 
    ...
    const { data } = await supabaseClient.from(CHAT_TABLE).select('*').order('created_at', { ascending: true }).limit(100); 
    box.innerHTML = ''; (data || []).forEach(renderChatMsg); box.scrollTop = box.scrollHeight; subscribeChat(); 
}
function subscribeChat() { 
    if (chatSubscribed) return; chatSubscribed = true; 
    supabaseClient.channel('redzone').on('postgres_changes', { event: 'INSERT', schema: 'public', table: CHAT_TABLE }, payload => { 
        renderChatMsg(payload.new); const box = document.getElementById('chatMessages'); box.scrollTop = box.scrollHeight; 
        if (!chatOpened) { unreadCount++; updateBadge(); } 
    }).subscribe(); 
}
function renderChatMsg(row) { 
    const box = document.getElementById('chatMessages'); 
    const loading = box.querySelector('.chat-loading'); if (loading) loading.remove(); 
    const own = row.sender === chatUser; 
    const cls = own ? 'chat-message-self' : (row.is_admin ? 'chat-message-admin' : 'chat-message-other'); 
    const time = new Date(row.created_at).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }); 
    const div = document.createElement('div'); div.className = `chat-message ${cls}`; 
    div.innerHTML = `${!own ? `<span class="chat-message-sender">${escapeHtml(row.sender)}</span>` : ''}<div class="chat-message-bubble">${row.reply_to ? `<div class="chat-message-reply">↩ ${escapeHtml(row.reply_to)}</div>` : ''}${escapeHtml(row.message)}</div><span class="chat-message-time">${time} <button class="reply-btn" onclick="setReply('${escapeHtml(row.sender)}')">↩</button></span>`; 
    box.appendChild(div); 
}
...
function toggleEmoji() { 
    ...
    const emojis = ['😀','😂','🤣','😊','😍','🥰','👍','🙏','👏','💪','👋','❤️','😢','😡','😭','🤝','😎','🥳','🎉','💖','🔥','✨','✅','🚫']; 
    picker.innerHTML = emojis.map(e => `<button class="emoji-btn" onclick="document.getElementById('chatInput').value+='${e}'">${e}</button>`).join(''); 
...
async function loadStok() { 
    ...
    const [barang, countRes] = await Promise.all([ fetchJsonData('v_stok_saat_ini'), supabaseClient.from('mutasi_stok').select('id', { count: 'exact', head: true }) ]); 
    globalStok.barang = barang || []; globalStok.totalTransaksi = countRes.count || 0; 
    renderStokStats(); renderStokGrid(); populateSelectBarang(); 
    const s = document.getElementById('searchStok'); if (s) s.oninput = () => renderStokGrid(); 
}
function renderStokStats() { 
    const el = document.getElementById('stokStats'); if (!el) return; 
    let aman = 0, menipis = 0, habis = 0; 
    globalStok.barang.forEach(b => { const sisa = Number(b.stok_saat_ini) || 0; if (sisa <= 0) habis++; else if (sisa <= (Number(b.stok_minimum) || 0)) menipis++; else aman++; }); 
    el.innerHTML = `<div class="info-stat-item"><span class="info-stat-icon">📦</span><span class="info-stat-label">Jenis Barang</span><span class="info-stat-value">${globalStok.barang.length}</span></div><div class="info-stat-item"><span class="info-stat-icon">🟢</span><span class="info-stat-label">Stok Aman</span><span class="info-stat-value">${aman}</span></div><div class="info-stat-item"><span class="info-stat-icon">🟠</span><span class="info-stat-label">Stok Menipis</span><span class="info-stat-value">${menipis}</span></div><div class="info-stat-item"><span class="info-stat-icon">🔴</span><span class="info-stat-label">Stok Habis</span><span class="info-stat-value">${habis}</span></div><div class="info-stat-item"><span class="info-stat-icon">🔄</span><span class="info-stat-label">Total Transaksi</span><span class="info-stat-value">${globalStok.totalTransaksi}</span></div>`; 
}
function renderStokGrid() { 
    const grid = document.getElementById('stokGrid'); if (!grid) return; 
    ...
    if (data.length === 0) { grid.innerHTML = '<div class="doc-empty"><div class="doc-empty-icon">📦</div><h3>Belum ada barang</h3><p>Klik "➕ Tambah Barang" untuk mulai mencatat.</p></div>'; return; } 
    grid.innerHTML = data.map(b => { 
        const sisa = Number(b.stok_saat_ini) || 0; const min = Number(b.stok_minimum) || 0; 
        ...
        return `<div class="info-card stok-card ${status}"><span class="stok-badge ${status}">${label}</span><h3>📦 ${escapeHtml(b.nama_barang)}</h3><p><span class="npsn-badge">${kodeSafe}</span></p><div class="info-row"><div class="info-label">Sisa Stok</div><div class="info-value stok-sisa">${sisa} ${escapeHtml(b.satuan || '')}</div></div><div class="info-row"><div class="info-label">Stok Minimum</div><div class="info-value">${min} ${escapeHtml(b.satuan || '')}</div></div><div class="info-row"><div class="info-label">Lokasi Rak</div><div class="info-value">${escapeHtml(b.lokasi_rak || '-')}</div></div><div style="display:flex;gap:8px;margin-top:12px;"><button class="info-toggle-btn" style="flex:1;margin-top:0;" onclick="openKartuStok('${kodeSafe}')">📇 Kartu Stock</button><button class="btn-hapus-stok" onclick="hapusBarang('${kodeSafe}')" title="Hapus barang ini">🗑️</button></div></div>`; 
    }).join(''); 
}
function populateSelectBarang() { const sel = document.getElementById('inputMutasiKode'); if (!sel) return; sel.innerHTML = globalStok.barang.map(b => `<option value="${escapeHtml(b.kode_barang)}">${escapeHtml(b.kode_barang)} - ${escapeHtml(b.nama_barang)}</option>`).join(''); }
async function openKartuStok(kode) { 
    const b = globalStok.barang.find(x => x.kode_barang === kode); if (!b) return; 
    document.getElementById('stokListView').style.display = 'none'; document.getElementById('stokSummaryView').style.display = 'none'; document.getElementById('kartuStokView').style.display = 'block'; 
    document.getElementById('kartuStokJudul').textContent = '📇 Kartu Stock: ' + b.nama_barang; 
    document.getElementById('ksNama').textContent = b.nama_barang || '-'; document.getElementById('ksKode').textContent = b.kode_barang || '-'; 
    document.getElementById('ksSatuan').textContent = b.satuan || '-'; document.getElementById('ksLokasi').textContent = b.lokasi_rak || '-'; 
    document.getElementById('ksMin').textContent = b.stok_minimum || 0; document.getElementById('ksSisa').textContent = (Number(b.stok_saat_ini) || 0) + ' ' + (b.satuan || ''); 
    const tbody = document.querySelector('#tableKartuStok tbody'); tbody.innerHTML = '<tr><td colspan="8" style="text-align:center;padding:20px;">⏳ Memuat riwayat...</td></tr>'; 
    ...
    tbody.innerHTML = (rows || []).map(m => { 
        const masuk = m.jenis === 'masuk' ? Number(m.jumlah) : 0; const keluar = m.jenis === 'keluar' ? Number(m.jumlah) : 0; sisa += masuk - keluar; 
...
function tutupKartuStok() { document.getElementById('kartuStokView').style.display = 'none'; document.getElementById('stokListView').style.display = 'block'; }
function toggleFormBarang() { const f = document.getElementById('formBarang'); const buka = (f.style.display === 'none' || f.style.display === ''); f.style.display = buka ? 'grid' : 'none'; document.getElementById('formMutasi').style.display = 'none'; }
function toggleFormMutasi() { const f = document.getElementById('formMutasi'); const buka = (f.style.display === 'none' || f.style.display === ''); f.style.display = buka ? 'grid' : 'none'; document.getElementById('formBarang').style.display = 'none'; if (buka) { document.getElementById('inputTanggal').value = new Date().toISOString().slice(0, 10); populateSelectBarang(); } }
async function simpanBarang(e) { 
    e.preventDefault(); if (!checkAuth()) { alert('🔒 Input stok terkunci! Masukkan PIN terlebih dahulu.'); showPINModal(); return; } 
    const payload = { kode_barang: document.getElementById('inputKode').value.trim().toUpperCase(), nama_barang: document.getElementById('inputNama').value.trim(), satuan: document.getElementById('inputSatuan').value.trim() || 'pcs', stok_minimum: Number(document.getElementById('inputMin').value) || 0, lokasi_rak: document.getElementById('inputLokasi').value.trim() }; 
    ...
    alert('✅ Barang baru berhasil ditambahkan!'); e.target.reset(); document.getElementById('formBarang').style.display = 'none'; loadStok(); 
}
async function simpanMutasi(e) { 
    e.preventDefault(); if (!checkAuth()) { alert('🔒 Input stok terkunci! Masukkan PIN terlebih dahulu.'); showPINModal(); return; } 
    const kode = document.getElementById('inputMutasiKode').value; const jenis = document.getElementById('inputJenis').value; const jumlah = Number(document.getElementById('inputJumlah').value); 
    ...
    if (jenis === 'keluar') { 
        ...
        const sisa = Number(rowStok?.stok_saat_ini || 0); if (jumlah > sisa) { alert(`❌ Stok tidak cukup! Sisa saat ini: ${sisa}`); return; } 
    } 
    const payload = { tanggal: document.getElementById('inputTanggal').value || new Date().toISOString().slice(0, 10), kode_barang: kode, jenis: jenis, jumlah: jumlah, no_nota: document.getElementById('inputNota').value.trim(), keterangan: document.getElementById('inputKet').value.trim(), petugas: document.getElementById('inputPetugas').value.trim() }; 
    ...
    alert('✅ Transaksi stok berhasil dicatat!'); e.target.reset(); document.getElementById('formMutasi').style.display = 'none'; loadStok(); 
}
async function hapusBarang(kode) { 
    ...
    const pesan = count > 0 ? 'Barang "' + b.nama_barang + '" memiliki ' + count + ' riwayat transaksi.\nSemua riwayat akan IKUT TERHAPUS permanen.\n\nLanjutkan?' : 'Hapus barang "' + b.nama_barang + '" dari daftar?'; 
    const yakin1 = await showConfirm(pesan, { title: 'Hapus Barang', confirmText: 'Ya, Hapus' }); if (!yakin1) return; 
    const yakin2 = await showConfirm('KONFIRMASI FINAL:\nBarang benar-benar akan dihapus permanen dan tidak bisa dikembalikan.', { title: 'Sekali Lagi!', confirmText: 'Hapus Permanen' }); if (!yakin2) return; 
    const { error: e1 } = await supabaseClient.from('mutasi_stok').delete().eq('kode_barang', kode); if (e1) { showToast('❌ Gagal hapus riwayat: ' + e1.message); return; } 
    const { error: e2 } = await supabaseClient.from('master_barang').delete().eq('kode_barang', kode); if (e2) { showToast('❌ Gagal hapus barang: ' + e2.message); return; } 
    showToast('✅ Barang beserta riwayatnya berhasil dihapus.'); loadStok(); 
}
async function hapusMutasi(id, kode) { 
    if (!checkAuth()) { showToast('🔒 Fitur hapus terkunci! Masukkan PIN terlebih dahulu.', 'warning'); showPINModal(); return; } 
    const yakin = await showConfirm('Hapus baris transaksi ini?\nSisa stok akan dihitung ulang otomatis.', { title: 'Hapus Transaksi', confirmText: 'Ya, Hapus' }); if (!yakin) return; 
    const { error } = await supabaseClient.from('mutasi_stok').delete().eq('id', id); if (error) { showToast('❌ Gagal hapus transaksi: ' + error.message); return; } 
    showToast('✅ Transaksi dihapus. Sisa stok dihitung ulang.'); await loadStok(); openKartuStok(kode); 
}
function toggleStokSummary() { 
    const listView = document.getElementById('stokListView'); const summaryView = document.getElementById('stokSummaryView'); const kartuView = document.getElementById('kartuStokView'); 
    if (summaryView.style.display === 'none' || summaryView.style.display === '') { listView.style.display = 'none'; kartuView.style.display = 'none'; summaryView.style.display = 'block'; renderStokSummary(); } 
    else { summaryView.style.display = 'none'; listView.style.display = 'block'; } 
    window.scrollTo({ top: 0, behavior: 'smooth' }); 
}
function renderStokSummary() { 
    const tbody = document.getElementById('tbodyRingkasanStok'); if (!tbody || globalStok.barang.length === 0) { tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;padding:20px;">Belum ada data barang</td></tr>'; return; } 
    ...
    tbody.innerHTML = sortedBarang.map((b, index) => { 
        const sisa = Number(b.stok_saat_ini) || 0; const min = Number(b.stok_minimum) || 0; 
        let status = 'AMAN'; let statusClass = 'status-aman'; 
        if (sisa <= 0) { status = 'HABIS'; statusClass = 'status-habis'; } else if (sisa <= min) { status = 'MENIPIS'; statusClass = 'status-menipis'; } 
...
function showToast(message, type) {
  ...
  const msg = document.createElement('span'); msg.className = 'toast-msg'; msg.textContent = message.replace(/^[✅❌⚠️🔴📢]\s*/, '');
...
function showConfirm(message, options) {
  ...
  return new Promise(function(resolve) {
    ...
    overlay.innerHTML = '<div class="confirm-modal">' + '<div class="confirm-icon">' + (options.danger === false ? '❓' : '⚠️') + '</div>' + '<h3>' + (options.title || 'Konfirmasi') + '</h3>' + '<p class="confirm-text"></p>' + '<div class="confirm-actions">' + '<button class="confirm-btn cancel">' + (options.cancelText || 'Batal') + '</button>' + '<button class="confirm-btn ok' + (options.danger === false ? '' : ' danger') + '">' + (options.confirmText || 'Ya, Lanjutkan') + '</button>' + '</div></div>';
    overlay.querySelector('.confirm-text').textContent = message;
    document.body.appendChild(overlay);
    requestAnimationFrame(function() { overlay.classList.add('active'); });
    function close(result) { overlay.classList.remove('active'); document.removeEventListener('keydown', onKey); setTimeout(function() { overlay.remove(); }, 300); resolve(result); }
