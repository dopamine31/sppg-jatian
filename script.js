...
async function loadRelawan() {
  ...
  if (data.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="8" style="text-align:center;padding:20px;">⚠️ Data tidak tersedia</td></tr>';
    return;
  }
  ...
  tbody.innerHTML = data.map((row, i) => {
    ...
    const waCell = waNumber
      ? `<a href="https://wa.me/${waNumber}" target="_blank">${escapeHtml(row['Nomor WA'])}</a>`
      : '-';

    const nikDisplay = maskNIK(row['NIK']);
    const nikClass = !checkAuth() ? 'masked-nik' : '';

    // Ambil URL KTP dari bucket "ktp"
    let ktpButton = '-';
    ...
    return `
      <tr>
        ...
        <td>${escapeHtml(row['Tanggal Lahir'])}</td>
        <td>${ktpButton}</td>
      </tr>`;
  }).join('');

  document
    .getElementById('searchRelawan')
    .addEventListener('input', (e) => filterTable(e, tbody));
