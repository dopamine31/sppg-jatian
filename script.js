...
const firebaseConfig = {
    apiKey: "AIzaSyBBTmEXeyfgE-DKg73OmkgroBp1_NLAOU",
...
let globalData = { relawan: [], sekolah: [], info: [], dokumen: [] };
...
function verifyPIN() {
    ...
    if (input === '2024') {
        ...
        renderRelawan(); renderSekolah(); loadSurat(); renderDokumen();
    } else {
...
function checkAndShow(sectionName) {
    const lockedSections = ['surat', 'dokumen', 'relawan', 'sekolah'];
    if (lockedSections.includes(sectionName) && !checkAuth()) { showPINModal(); return; }
    showSection(sectionName);
}
function checkExternalLink(event) {
    if (!checkAuth()) { event.preventDefault(); showPINModal(); return false; }
    return true;
}
...
// ===== DATA LOADERS & RENDERERS (OPTIMIZED CACHE) =====
...
async function loadDokumen() {
    ...
    globalData.dokumen = data;
    renderDokumen();
}
function renderDokumen() {
    const grid = document.getElementById('docGrid'); if (!grid) return;
    let data = globalData.dokumen || [];
    if (data.length === 0) { grid.innerHTML = '<div class="doc-empty"><div class="doc-empty-icon">📚</div><h3>Belum ada dokumen</h3></div>'; return; }
    if (currentFilter !== 'all') { data = data.filter(doc => doc['Kategori']?.toLowerCase() === currentFilter); }
    const searchTerm = document.getElementById('searchDokumen')?.value.toLowerCase() || '';
    if (searchTerm) { data = data.filter(doc => doc['Judul']?.toLowerCase().includes(searchTerm) || doc['Deskripsi']?.toLowerCase().includes(searchTerm)); }
    ...
    grid.innerHTML = data.map((doc) => {
        ...
        return `<div class="doc-item" data-category="${category}">
            ...
            <div class="doc-meta"><div class="doc-meta-item"><span>📅</span><span>Update: ${tanggalUpdate}</span></div>${doc['Ukuran'] ? `<div class="doc-meta-item"><span>💾</span><span>${doc['Ukuran']}</span></div>` : ''}</div>
...
// =====================================================================
let chatUserName = ''; let userColor = ''; let chatRoomActive = false; let chatListenersAttached = false; let currentUserId = ''; let chatSessionId = '';
function toggleChat() {
    const chatWindow = document.getElementById('chatWindow');
    if (chatWindow.style.display === 'none') {
        chatWindow.style.display = 'flex';
        if (chatUserName) { enterChatRoom(); } else { document.getElementById('chatNameForm').style.display = 'flex'; document.getElementById('chatRoom').style.display = 'none'; }
    } else {
        chatWindow.style.display = 'none'; if (chatRoomActive) { leaveChatRoom(); }
    }
}
function startChat() {
    const nameInput = document.getElementById('chatNameInput'); const name = nameInput.value.trim();
    if (!name) { alert('Silakan masukkan nama Anda terlebih dahulu.'); return; }
...
function updateUserPresence() {
    ...
    const userData = { name: chatUserName, color: userColor, isAdmin: checkAuth(), lastSeen: firebase.database.ServerValue.TIMESTAMP };
...
function setupChatListeners() {
    ...
    chatMessagesRef.limitToLast(50).on('child_added', (snapshot) => { const message = snapshot.val(); if (message) { appendMessage(message); } });
    onlineUsersRef.on('value', (snapshot) => { const users = snapshot.val() || {}; const count = Object.keys(users).length; document.getElementById('onlineCount').textContent = count; });
...
function addSystemMessage(text) {
    const messagesContainer = document.getElementById('chatMessages');
    const msgDiv = document.createElement('div'); msgDiv.className = 'chat-system-msg'; msgDiv.textContent = text;
    messagesContainer.appendChild(msgDiv); messagesContainer.scrollTop = messagesContainer.scrollHeight;
}
function appendMessage(message) {
    const messagesContainer = document.getElementById('chatMessages');
    const loading = messagesContainer.querySelector('.chat-loading'); if (loading) loading.remove();
    ...
    if (isAdmin) { msgClass = 'chat-message-admin'; } else if (isSelf) { msgClass = 'chat-message-self'; }
    ...
    const senderNameHtml = isAdmin ? `${escapeHtml(message.sender)} <span class="admin-badge">ADMIN</span>` : escapeHtml(message.sender);
    const bubbleStyle = (!isAdmin && isSelf && bubbleColor) ? `style="background: ${bubbleColor}; color: #fff;"` : '';
    const msgHTML = `<div class="chat-message ${msgClass}"><div class="chat-message-sender" style="color: ${msgColor};">${senderNameHtml}</div><div class="chat-message-bubble" ${bubbleStyle}>${escapeHtml(message.text)}</div><div class="chat-message-time">${timeStr}</div></div>`;
    messagesContainer.insertAdjacentHTML('beforeend', msgHTML);
    messagesContainer.scrollTop = messagesContainer.scrollHeight;
    const chatWindow = document.getElementById('chatWindow');
    if (chatWindow.style.display === 'none' && !isSelf) { const badge = document.getElementById('chatBadge'); badge.classList.add('show'); setTimeout(() => badge.classList.remove('show'), 3000); }
}
function formatTimestamp(timestamp) {
    const date = new Date(timestamp); const now = new Date(); const isToday = date.toDateString() === now.toDateString();
    const timeStr = date.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' });
    if (isToday) { return timeStr; } else { return date.toLocaleDateString('id-ID', { day: 'numeric', month: 'short' }) + ' ' + timeStr; }
}
function sendMessage() {
    const input = document.getElementById('chatMessageInput'); const text = input.value.trim();
    if (!text) return;
    if (!chatUserName) { alert('Silakan masukkan nama terlebih dahulu.'); return; }
    const messageData = { sender: chatUserName, sessionId: chatSessionId, text: text, color: userColor, isAdmin: checkAuth(), timestamp: firebase.database.ServerValue.TIMESTAMP };
    chatMessagesRef.push(messageData).then(() => { input.value = ''; input.focus(); }).catch((error) => { console.error('Error sending message:', error); alert('Gagal mengirim pesan. Silakan coba lagi.'); });
}
...
window.addEventListener('DOMContentLoaded', async () => {
    ...
    // Event Listeners (Dipindahkan ke DOMContentLoaded agar tidak menumpuk / memory leak)
    document.getElementById('searchSekolah').addEventListener('input', (e) => filterTable(e, document.querySelector('#tableSekolah tbody')));
    document.getElementById('searchRelawan').addEventListener('input', (e) => filterTable(e, document.querySelector('#tableRelawan tbody')));
    document.getElementById('searchSurat').addEventListener('input', (e) => {
        const q = e.target.value.toLowerCase();
        document.getElementById('cardSurat').querySelectorAll('.info-card').forEach(card => { card.style.display = card.textContent.toLowerCase().includes(q) ? '' : 'none'; });
    });
    document.getElementById('searchDokumen').addEventListener('input', renderDokumen); // OPTIMASI SEARCH
    document.getElementById('searchInfo').addEventListener('input', renderInfoBoard);
    ...
    if (btnDownload) {
        btnDownload.addEventListener('click', async () => {
            ...
            btnDownload.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Memproses...'; btnDownload.disabled = true;
            try {
    ...
    window.addEventListener('beforeunload', () => { if (currentUserId && chatRoomActive) { onlineUsersRef.child(currentUserId).remove(); } detachChatListeners(); });
});